...
$ docker run --name mydb -p 3306:3306 -e MYSQL_USER=codify -e MYSQL_PASSWORD=password -e MYSQL_ROOT_PASSWORD=password -e MYSQL_DATABASE=mycodifydb -d mysql:8.1

# Check if mysql is working fine
$ docker exec -it mydb mysql -uroot -ppassword
  mysql> show databases;
  mysql> use mycodifydb;
  mysql> show tables;
  mysql> select * from books;

# Deploy the application
$ nohup java -jar target/codify-springboot-mysql-9739110917.jar 2>&1 &
